...
from knowledge_reuse.sources.lean_mathlib.layout import EXCLUSIONS_PATH, RESULTS_ROOT, ROOT


MATHLIB = ROOT / "vendor" / "mathlib4"
RESULT = RESULTS_ROOT / "inventory.json"
...
def load_exclusion_rules() -> list[dict[str, str]]:
    config = tomllib.loads(EXCLUSIONS_PATH.read_text())
    return config["rule"]
