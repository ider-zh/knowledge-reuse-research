...
def validate(run_kind: str) -> dict[str, Any]:
    ...
    cache_path = DATA_ROOT / "cache" / "analysis.duckdb"
    ...
    try:
        ...
        failures = dict(con.execute((SQL_ROOT / "validation.sql").read_text()).fetchall())
        ...
        expected_modules = (
            ...
            if run_kind == "smoke"
            else json.loads((RESULTS_ROOT / "inventory.json").read_text())["summary"]["included"]
        )
    ...
    if run_kind == "smoke":
        ...
        shard_plan = json.loads((RESULTS_ROOT / "shard-plan.json").read_text())
